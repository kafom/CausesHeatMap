...
function updateBarHeader()
{
    var str = [currentAge, currentCause];
    var headerData = ["x"];
    barHeaderLabel.selectAll("text")
            .data(headerData)
            .enter()
            .append("text")
    ...
    labelHM.append("g")
     ...
     labelHM.append("g")
            ...
            .enter().append("text")
                ...
                .attr("dy", ".15em")
                .on("mouseover", function (d,i){
                    showCauseName(d,i,this);
                })
                .on("mouseout", function (d,i){
                    //showCauseName(d,i,this);
                    nameTooltip.hideTooltip();
                });
...
function show_detailsBar(data,i,element)
{
	d3.select(element).attr("stroke", "black");
	var content = "<span class=\"name\">Value:</span><span class=\"value\"> " + data.value + "</span><br/>";
	//content +="<span class=\"name\">Age Group:</span><span class=\"value\"> " + data.age + "</span><br/>";

	tooltip.showTooltip(content, d3.event, 10, 5);
}
...
function showCauseName(data,i,element)
{
 //   d3.select(element).attr("stroke", "black");
    var content = "<span class=\"name\"></span><span class=\"value\"> " + data + "</span><br/>";
    nameTooltip.showTooltip(content,d3.event, -10, -23);
}

function hide_details(data, i, element) {           //return d3.rgb(colorScale(d.value)).darker();}
    d3.select(element).attr("stroke", function(d) { return d3.rgb(colorScale(d.value));} );
    tooltip.hideTooltip();
}
